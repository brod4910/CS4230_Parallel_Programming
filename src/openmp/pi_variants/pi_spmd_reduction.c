...
int main ()
{
  ...
  for (j=1;j<=MAX_THREADS ;j++) {
    ...
    sum = 0.0;
    pi = 0.0; // If you don't clear this here, of course it will accumulate !!
          #pragma omp parallel
    {
      int i;
      ...
      // Try declaring pi here and it will not be seen by the final printf

      if (id == 0)
	printf(" num_threads = %d",numthreads);

      // The threads share work
      // Divide the iteration sapce "equally"
      // (static schedule)
      //
...
// http://bisqwit.iki.fi/story/howto/openmp/ pretty good
